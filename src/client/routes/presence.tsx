...
export default function PresenceRoute() {
  ...
  // Legacy redirect: the "Paramètres" sub-tab used to live here as
  // /presence?tab=settings. It now lives under /settings?tab=presence
  // so config knobs are co-located with the rest of the dashboard
  // settings. Catch any external bookmark / muscle-memory URL once on
  // mount and forward it; runs only when the query param is present.
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    if (params.get('tab') === 'settings') {
      navigate('/settings?tab=presence', { replace: true });
    }
  }, [navigate]);
  ...
  }, [tab, loadFeed, loadSources, loadStats, statsWindow]);

  // Preload sources (for badge count) + connection status (for scan gating
  // of Auto-post / Assist buttons + the X-source disabled state in
  // SourcesView). The full connection-management UI now lives under
  // /settings?tab=presence — we only need the boolean status here.
  useEffect(() => {
    void loadSources();
    void loadStatus();
